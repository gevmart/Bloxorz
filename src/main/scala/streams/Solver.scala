...
trait Solver extends GameDef {
   ...
   */
  // TODO check, not sure what the method wants to do
  ...
  if(initial.isEmpty) initial
    ...
    val more = for {
      in <- initial
      next <- newNeighborsOnly(neighborsWithHistory(in._1, in._2), explored)
    } yield next

    initial ++ from(more, explored ++ (more map (m => m._1)))
  }
